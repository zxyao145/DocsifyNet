...
namespace DocsifyNet.Module
{
    ...
    public class SidebarCreator
    {
        ...
        private string GenMdLink(string relativeFileName)
        {
            string linkAddress = "";
            ...
            return $"- [{linkShowName}]({linkAddress}){Environment.NewLine}";
        }
        ...
        private async Task<List<string>> WriteToSidebarFileAsync(DirNode node, string relativeDirName = "")
        {
            ...
            if (node.Dirs.Any())
            {
                ...
                foreach (var subDir in node.Dirs)
                {
                    ...
                    curPathFiles.Add(GenMdLink($"{relativeDirName}{subDir.CurDirName}"));
            ...
            int level = 0)
        {
            ...
            DirNode curDirNode = new DirNode()
            {
                Parent = parentNode,
                RelativeLevel = level,
                CurDirName = level == 0 ? "" : Path.GetFileName(dirName) ?? "",
                Files = fileList,
            };
